 ...
 * of the bundle, so adding a `ClaudeSessionsBundle_<language>.properties` file - even a partial one -
 * is all it takes to support one more language.
 *
 * It lives in `core/` rather than in `ui/` because the warnings, the progress texts and the import
 * diagnosis are produced here: see [displayLanguage] for how the IDE language reaches an IntelliJ
 * free layer.
 */
object ClaudeSessionsBundle {

    private const val BUNDLE = "messages.ClaudeSessionsBundle"
    ...
    /** The locale [message] currently resolves to, `Locale.ROOT` meaning the English base bundle. */
    fun effectiveLocale(): Locale =
        candidateLocales().firstNotNullOfOrNull { translation(it)?.locale } ?: Locale.ROOT
    ...
    private fun bundle(locale: Locale): ResourceBundle? = try {
        ResourceBundle.getBundle(BUNDLE, locale, ClaudeSessionsBundle::class.java.classLoader, control)
    } catch (_: MissingResourceException) {
        null
    }
}
